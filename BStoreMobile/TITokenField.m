...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    BOOL isContained = NO;
    for (TIToken *tt in self.tokenField.tokens) {
        if ([tt.title isEqualToString:token.title]) {
            isContained = YES;
            break;
        }
    }
...
- (void)didEndEditing {
	...
	if (_removesTokensOnEndEditing){
		...
		if (_tokens.count){
            ...
            CGSize untokSize = [untokenized sizeWithAttributes:attributes];
        #else
			...
			if (_tokens.count > 1 && untokSize.width > availableWidth){
				untokenized = [NSString stringWithFormat:@"%lu 本书", (unsigned long)titles.count];
			}
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000
    NSDictionary *attr = @{NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:14]};
    [_title drawInRect:textBounds withAttributes:attr];
#else
